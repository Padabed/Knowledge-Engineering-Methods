...
for epoch in range(2000):
    X = np.dot(W1, P.reshape(1, -1)) + np.dot(B1, np.ones((1, len(P))))
    A1 = np.maximum(X, 0)
    A2 = np.dot(W2, A1) + B2
    ...
    dW1 = lr * (np.exp(X) / (np.exp(X) + 1)) * np.dot(E1, P.reshape(1, -1).T)
    dB1 = lr * (np.exp(X) / (np.exp(X) + 1)) * np.dot(E1, np.ones_like(P).T)
